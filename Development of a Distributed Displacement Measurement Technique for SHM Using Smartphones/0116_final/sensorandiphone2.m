...
time=time-37.5+0.07;%传感器改时间
h=plot(time,(vari3-0.1)*1.5,'b');%%%%%%%%%
hold on;
grid on;
...
t00 = Minute*60+Second-512.78;%这一句在合成时须作人工调整；
interval = t00;
...
dx3=(dx-dx2)*(-0.5);
%}
interval=interval-37.5+5.34+10.84-2.097;%手机改时间用
plot(interval,-dy*0.75,'r');%%%%%%%%%
%x=interval;
%axis([0,5,-2.5,2.5]);%坐标轴显示范围，这个先注释，改完时间后再去掉注释
axis([-3,3,-3,3]);
%datetick('x','HH:MM:SS');
set(gca,'FontName','Times New Roman','FontSize',10);
set(gcf,'color','w');
